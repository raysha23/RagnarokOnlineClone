...
import { loadSkillState } from "../state/skill-state.js";
...
export function restoreJobState() {
  ...
  const baseLevel = parseInt(
    params.get("baseLevel") || character.baseLevel || "1",
    10,
  );
  ...
  const jobLevel = parseInt(
    params.get("jobLevel") || character.jobLevel || "1",
    10,
  );
  ...
  if (!gameData.characters[job]) {
    console.warn(
      `[RESTORE] No character data for job '${job}', initializing...`,
    );
